...
app.post(
  ...
  (req, res) => {
    ...
    if (
      ...
      req.files.attachment.constructor == Array
    )
    ...
    if (req.body.event && req.body.part) {
      ...
      require("./service/hades")
        .getPart(req.body.event, req.body.part)
        .then(emails => {
          sendMail(
            emails,
            req.body.subject,
      ...
      fs.readFile(
        path.join(__dirname, req.files.csv[0].path),
        "utf8",
        (err, data) => {
          ...
          if (emails.length > 0) {
            ...
            return res
              .status(404)
              .json({ err: "No mailid found under email/emails field" });
          }
        }
      );
